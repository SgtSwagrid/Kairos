...
object Env:
  ...
  /**
    * Where polls are mirrored to disk so that a restart does not lose one, or
    * [[None]] to keep them in memory alone.
    */
  val DATA_FILE: Option[Path] = env("DATA_FILE")
    .orElse(jvm("data.file"))
    .orElse(Some("data/polls.json"))
    .filterNot(_ == "none")
    .map(Paths.get(_))
